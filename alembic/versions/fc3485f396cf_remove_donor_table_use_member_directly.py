...
def upgrade():
    ...
    op.create_foreign_key('offerings_member_id_fkey', 'offerings', 'members', ['member_id'], ['id'])
    
    op.alter_column('receipts', 'member_id', nullable=False)
    op.create_foreign_key('receipts_member_id_fkey', 'receipts', 'members', ['member_id'], ['id'])
    
    # Update receipt_snapshots to use member instead of donor
    op.add_column('receipt_snapshots', sa.Column('member_name', sa.String(), nullable=True))
    op.add_column('receipt_snapshots', sa.Column('member_rrn_masked', sa.String(), nullable=True))
    op.add_column('receipt_snapshots', sa.Column('member_address', sa.String(), nullable=True))
...
def downgrade():
    ...
    op.add_column('receipt_snapshots', sa.Column('donor_address', sa.String(), autoincrement=False, nullable=True))
    op.add_column('receipt_snapshots', sa.Column('donor_rrn_masked', sa.String(), autoincrement=False, nullable=True))
    op.add_column('receipt_snapshots', sa.Column('donor_name', sa.String(), autoincrement=False, nullable=False))
    
    # Revert receipts
    op.drop_constraint('receipts_member_id_fkey', 'receipts', type_='foreignkey')
    op.drop_column('receipts', 'member_id')
    op.add_column('receipts', sa.Column('donor_id', sa.Integer(), autoincrement=False, nullable=False))
    op.create_foreign_key('receipts_donor_id_fkey', 'receipts', 'donors', ['donor_id'], ['id'])
    
    # Revert offerings
    op.drop_constraint('offerings_member_id_fkey', 'offerings', type_='foreignkey')
    op.drop_column('offerings', 'member_id')
    op.add_column('offerings', sa.Column('donor_id', sa.Integer(), autoincrement=False, nullable=False))
    op.create_foreign_key('offerings_donor_id_fkey', 'offerings', 'donors', ['donor_id'], ['id'])