...
def upgrade() -> None:
    ...
    op.create_table('daily_verses',
        ...
        sa.Column('reference', sa.String(length=100), nullable=False, comment='성경 구절 출처 (예: 시편 23:1)'),
        sa.Column('is_active', sa.Boolean(), nullable=True, server_default='true', comment='활성 상태'),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=True),
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=True),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_daily_verses_id'), 'daily_verses', ['id'], unique=False)
    op.create_index(op.f('ix_daily_verses_is_active'), 'daily_verses', ['is_active'], unique=False)
