...
from app.schemas.ai_agent import (
    ChurchDatabaseConfigCreate,
    ChurchDatabaseConfigUpdate,
...
def read_church_profile(
    *,
    ...
    # Get agent count
    agents_count = db.query(AIAgent).filter(AIAgent.church_id == church.id).count()

    # Get database config status
    db_config = (
        db.query(ChurchDatabaseConfig)
        .filter(
    ...
    monthly_messages = (
        ...
        .join(models.ChatHistory, ChatMessage.chat_history_id == models.ChatHistory.id)
        .filter(
    ...
    total_cost = openai_service.calculate_cost(
    ...
    return {
        ...
        "data": {
            ...
            "monthly_usage": {
                ...
                "remaining_quota": max(
                    0, (church.monthly_token_limit or 100000) - total_tokens
                ),
            },
        },
    }
...
async def update_gpt_config(
    *,
    ...
    if current_user.role not in ["admin"]:
        raise HTTPException(
            status_code=403, detail="Only admins can update GPT configuration"
        )
    ...
    return {"success": True, "message": "GPT configuration updated successfully"}
...
async def create_database_config(
    *,
    ...
    if current_user.role not in ["admin"]:
        raise HTTPException(
            status_code=403, detail="Only admins can configure database connection"
        )

    # Check if config already exists
    existing_config = (
        db.query(ChurchDatabaseConfig)
        .filter(ChurchDatabaseConfig.church_id == current_user.church_id)
        .first()
    )
    ...
    if existing_config:
        ...
        config = ChurchDatabaseConfig(**config_dict, church_id=current_user.church_id)
