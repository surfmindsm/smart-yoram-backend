...
def create_family_relationship(
    *,
    ...
    related_member = db.query(models.Member).filter(
    ...
    existing = db.query(models.FamilyRelationship).filter(
        ...
        models.FamilyRelationship.related_member_id == relationship_in.related_member_id
    ).first()
...
@router.get("/relationships/{member_id}", response_model=List[schemas.FamilyRelationship])
def get_member_relationships(
    *,
    ...
    relationships = db.query(models.FamilyRelationship).filter(
...
def get_family_tree(
    *,
    ...
    """
    root_member = db.query(models.Member).filter(
        models.Member.id == member_id,
        models.Member.church_id == current_user.church_id
    ).first()
    ...
    family_relationships = db.query(models.FamilyRelationship).filter(
        or_(
            models.FamilyRelationship.member_id == member_id,
    ...
    for rel in family_relationships:
        if rel.member_id == member_id:
            ...
            reverse_rel = db.query(models.FamilyRelationship).filter(
    ...
    if related_member_ids:
        family_members = db.query(models.Member).filter(
            models.Member.id.in_(related_member_ids)
        ).all()
        
        for member in family_members:
            family_members_data.append(schemas.FamilyTreeMember(
                id=member.id,
                name=member.name,
                profile_photo_url=member.profile_photo_url,
                relationship_type=relationship_map.get(member.id),
                date_of_birth=member.birthdate.isoformat() if member.birthdate else None,
    ...
    root_member_data = schemas.FamilyTreeMember(
        ...
        date_of_birth=root_member.birthdate.isoformat() if root_member.birthdate else None,
        phone_number=root_member.phone
    )
    
    return schemas.FamilyTree(
        root_member=root_member_data,
...
def delete_family_relationship(
    *,
    ...
    """
    relationship = db.query(models.FamilyRelationship).filter(
    ...
    reverse_relationship = db.query(models.FamilyRelationship).filter(
