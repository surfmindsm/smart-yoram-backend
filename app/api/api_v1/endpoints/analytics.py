...
def read_usage_analytics(
    *,
    ...
    current_period_stats = (
        ...
        .filter(
            ChatHistory.church_id == church.id, ChatMessage.created_at >= start_date
        )
        .first()
    )
    ...
    cost_usd = openai_service.calculate_cost(
        total_tokens, church.gpt_model or "gpt-4o-mini"
    )
    ...
    if period in ["month", "current_month"]:
        ...
        for stat in daily_stats:
            daily_cost = openai_service.calculate_cost(
                stat.tokens or 0, church.gpt_model or "gpt-4o-mini"
            )
            daily_usage.append(
                {
    ...
    agent_stats = (
        ...
        .join(ChatHistory, AIAgent.id == ChatHistory.agent_id)
        .join(ChatMessage, ChatHistory.id == ChatMessage.chat_history_id)
        .filter(AIAgent.church_id == church.id, ChatMessage.created_at >= start_date)
...
def read_agent_statistics(
    *,
    ...
    """
    query = db.query(AIAgent).filter(AIAgent.church_id == current_user.church_id)
    ...
    for agent in agents:
        ...
        stats = (
            ...
            .join(ChatHistory, ChatMessage.chat_history_id == ChatHistory.id)
            .filter(ChatHistory.agent_id == agent.id)
            .first()
        )

        # Get conversation count
        conversation_count = (
            db.query(ChatHistory).filter(ChatHistory.agent_id == agent.id).count()
        )
    ...
    *,
    ...
    for trend in trends:
        cost = openai_service.calculate_cost(
            trend.tokens or 0, church.gpt_model or "gpt-4o-mini"
        )
    ...
    if len(trend_data) >= 7:
        ...
        prev_week = (
            sum(t["tokens"] for t in trend_data[-14:-7]) if len(trend_data) >= 14 else 0
        )
...
def read_top_queries(
    *,
    ...
    top_queries = sorted(keyword_counts.items(), key=lambda x: x[1], reverse=True)[
        :limit
    ]
    ...
    return {
        ...
        "data": {
            "top_queries": [
                {"keyword": keyword, "count": count} for keyword, count in top_queries
            ],
            "period_days": days,
