...
def get_attendance_summary(
    *,
    ...
    avg_attendance_rate = (avg_attendance / total_members * 100) if total_members > 0 else 0
...
def get_attendance_by_member(
    *,
    ...
    total_days = db.query(func.count(func.distinct(models.Attendance.attendance_date))).filter(
        models.Attendance.church_id == current_user.church_id,
        models.Attendance.attendance_date >= start_date,
        models.Attendance.attendance_date <= end_date,
        models.Attendance.attendance_type == attendance_type
    ...
    for stat in attendance_stats:
        attendance_rate = (stat.attendance_count / total_days * 100) if total_days > 0 else 0
...
def get_member_demographics(
    *,
    ...
    ).group_by(models.Member.position).all()
    ...
    ).group_by(models.Member.district).all()
    ...
    return {
        ...
        'position_distribution': [
            {'position': stat.position, 'count': stat.count}
            for stat in position_stats
        ],
        'district_distribution': [
            {'district': stat.district, 'count': stat.count}
