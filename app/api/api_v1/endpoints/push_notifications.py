...
async def unregister_device(
    ...
    success = await PushNotificationService.unregister_device(
        db=db, device_token=device_token
    )
    if not success:
        raise HTTPException(
...
def get_my_devices(
    ...
    devices = (
        db.query(UserDevice)
        .filter(UserDevice.user_id == current_user.id, UserDevice.is_active == True)
        .all()
    )
    return devices
...
async def send_notification(
    ...
    target_user = (
        db.query(User)
        .filter(
            User.id == notification.user_id, User.church_id == current_user.church_id
        )
        .first()
    )
...
async def send_batch_notification(
    ...
    users_count = (
        db.query(User)
        .filter(
            User.id.in_(notification.user_ids), User.church_id == current_user.church_id
        )
        .count()
    )
