...
async def get_containers(current_user: models.User = Depends(check_system_admin)):
    """
...
async def websocket_logs(
    ...
    try:
        ...
        async for log_line in docker_logs_service.get_logs(
            ...
            await websocket.send_json(
                {"type": "log", "container": container_name, "message": log_line}
            )
...
async def clear_logs(
    container_name: str, current_user: models.User = Depends(check_system_admin)
):
    """
    ...
    if not success:
        raise HTTPException(
            status_code=500, detail=f"Failed to restart container {container_name}"
        )
...
async def get_logs_summary(
    ...
    for container in containers:
        try:
            ...
            summary[container] = {"error": str(e)}

    return {"since_minutes": since_minutes, "summary": summary}
