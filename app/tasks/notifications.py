...
from app.models.push_notification import (
    PushNotification, NotificationType, NotificationStatus,
    UserDevice, NotificationRecipient
)
    ...
    try:
        ...
        if notification.target_type == "all":
            user_ids = db.query(User.id).filter(
                User.church_id == notification.church_id,
                User.is_active == True
            ).all()
        ...
        logger.info(f"Notification {notification_id} sent: {total_sent} success, {total_failed} failed")
    ...
    try:
        ...
        for church in churches:
            ...
            for service in services:
                service_times.append(f"{service.name}: {service.start_time.strftime('%H:%M')}")
            ...
            notification = PushNotification(
                ...
                body=body,
                target_type="all",
                data={"type": "worship_reminder", "date": datetime.now().strftime("%Y-%m-%d")}
            )
            db.add(notification)
            db.commit()
...
def cleanup_expired_tokens():
    ...
    try:
        ...
        expired_devices = db.query(UserDevice).filter(
            UserDevice.last_used_at < cutoff_date
        ).all()
...
def retry_failed_notification(self, recipient_id: int):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Successfully retried notification for recipient {recipient_id}")
