...
class RedisClient:
    def __init__(self):
        ...
        try:
            ...
            if hasattr(redis, "Retry") and hasattr(redis, "ExponentialBackoff"):
                ...
                logger.warning(
                    "Redis.Retry not available, using legacy retry configuration"
                )
                redis_params.update(
                    {
                        "retry_on_error": [redis.ConnectionError, redis.TimeoutError],
            ...
            logger.warning(
                "Redis is not available. Push notifications and caching will be disabled."
            )
            self.connected = False
    ...
    def store_device_token(
        self, user_id: int, device_token: str, platform: str, ttl: int = 86400 * 30
    ):
        ...
        value = {"platform": platform, "user_id": user_id, "device_token": device_token}
    ...
    def check_rate_limit(
        self, user_id: int, limit: int = 100, window: int = 3600
    ) -> bool:
    ...
    def set_notification_status(
        self, notification_id: int, status: str, ttl: int = 86400
    ):
