...
def get_recent_announcements(db: Session, church_id: int, limit: int = 10) -> List[Dict]:
    """
    ...
    try:
        ...
        ).order_by(desc(Announcement.created_at)).limit(limit).all()
    ...
    if context_data.get('worship_services'):
        ...
        if services:
            ...
            for service in services[:3]:  # Limit to next 3 services
                date_str = service['worship_date'].split('T')[0] if service['worship_date'] else "미정"
                context_parts.append(
