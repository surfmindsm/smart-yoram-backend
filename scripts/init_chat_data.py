    ...
    try:
        ...
        if not agents:
            ...
            for agent_data in default_agents:
                agent = AIAgent(church_id=church.id, **agent_data)
                db.add(agent)
        ...
        for agent in agents:
            logger.info(
                f"  - Agent ID: {agent.id}, Name: {agent.name}, Category: {agent.category}"
            )
        ...
        histories = (
            ...
            .filter(ChatHistory.church_id == church.id, ChatHistory.user_id == user.id)
            .all()
        )
        ...
        if not histories and agents:
            ...
            logger.info(
                f"Created chat history ID: {history.id} with agent ID: {first_agent.id}"
            )
        else:
            logger.info(f"Found {len(histories)} existing chat histories:")
            for history in histories:
                logger.info(
                    f"  - History ID: {history.id}, Agent ID: {history.agent_id}, Title: {history.title}"
                )
