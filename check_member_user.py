...
def check_member_user():
    ...
    try:
        ...
        print("-" * 80)
        print(
            f"{'ID':<5} {'Name':<20} {'Email':<30} {'User ID':<10} {'Has Account':<12}"
        )
        print("-" * 80)
        ...
        for member in members:
            ...
            print(
                f"{member.id:<5} {member.name:<20} {member.email or 'N/A':<30} {member.user_id or 'None':<10} {has_account:<12}"
            )
        ...
        for user in users:
            print(
                f"{user.id:<5} {user.email:<30} {user.username:<20} {user.church_id:<10}"
            )
